...
2. Extract it in the project root, so you get this path:
	`things_data/osfstorage/images_THINGS/object_images/`
3. Run the parser:

	```powershell
	python data_parser.py
	```
    This creates `all_images.pkl` in the project root.

    `all_images.pkl` is a dictionary:
    - key = object class folder name
